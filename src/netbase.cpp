...
//	dissected[subject]=1;
	...
	if (type >= 0 && len - type > 2) {
		...
		if (original != subject) addStatement(original, in, ort, false);
...
Node * findWord(int context, const char* word, bool first) {	//=false
	...
	for (int i=0; i < c->nodeCount; i++) {
		...
		//			break;
		//		}
		if (n == null || n->name == null || word == null || n->id == 0 || n->context == 0) continue;
		if (n->id >= maxNodes) {
			printf("n>=maxNodePointer ???");
			continue;
		}
		if (!checkNode(n, n->id, true, true)) continue;
...
Statement * findStatement(Node* subject, Node* predicate, Node* object, int recurse, bool semantic, bool symmetric,
