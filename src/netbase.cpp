...
void dissectParent(Node *subject, bool checkDuplicates) {
	...
	bool plural = len > 2 and (char) str[len - 1] == 's' and (char) str[len - 2] != 's' and
...
//			addStatement(subject, Label, b, true);
	...
	type = (int) str.find("(");
	if (type > 0 and length - type > 2) {        // not (030)4321643 !
		int to = (int) str.find(")");
		...
		else word = getThe(str.substr(to + 1, length - 1).data()); //deCamel
...
//		addStatement(subject, Instance, clazz, true);
	...
	type = (int) str.find(". ");
	if (type >= 0 and length - type > 2 and isNumber(str.data())) {
...
NodeVector *findWordsByName(int context, const char *word, bool first, bool containsWord) {    //=false
	...
	for (int i = -propertySlots; i < c->nodeCount; i++) {
		...
		if (good) {
			...
			else if (eq(abstract->name, n->name))
...
//			break;// todo : make sure statements are ordered!
		...
		bool is_symmetric =
				symmetric or s->Predicate() == Synonym or predicate == Synonym or s->Predicate() == Antonym or
				predicate == Antonym;
...
NodeVector showNodes(NodeVector all, bool showStatements, bool showRelation, bool showAbstracts) {
	...
	for (int i = 0; i < size and i < resultLimit; i++) {
		...
		if (i > 0 and showRelation) {
			...
			N n = findDirectRelation(all[i - 1], node);
			if (!r or !n) p("? -> ? \n");
			else {
				pf("$%d-> ", r->id());
				pf("%s\n", n->name);
